...
float magCalibration[3] = {
  0, 0, 0}
, magbias[3] = {
  0, 0, 0};  // Factory mag calibration and mag bias
float gyroBias[3] = {
  0, 0, 0}
, accelBias[3] = {
...
PID PID_Y(&inY, &outY, &refY, 35, 0, 0, DIRECT);    // Y axis PID 1&3
PID PID_X(&inX, &outX, &refX, 35, 0, 0, DIRECT);    // X Axis PID 2&4
...
void setup()
{
  ...
  PID_X.SetSampleTime(30);
  PID_Y.SetSampleTime(30);   
  ...
  if (currentMillis - previousMillis >= interval)      // if the right amount of time has passed then...
  {
    ...
    pwm4 = pwm2;  
      ...
      if(SerialDebug) {
        ...
        Serial.print("oPWM2 = ");
        Serial.print(opwm2);
        Serial.println(inrange(pwm2, opwm2));
      }

//    if (!inrange(pwm1, opwm1)){
//      if (pwm1 != opwm1){
...
boolean inrange(int pwm, int opwm) {
  int local_opwm = abs(opwm);
  int local_pwm = abs(pwm);
  Serial.print("l_OPWM = ");
  Serial.print(local_opwm);
  Serial.println();
  Serial.print("l_PWM = ");
  Serial.print(local_pwm);
  Serial.println();
  float diff = abs(local_opwm - local_pwm) / local_opwm;
  if (diff < p) {
    return true;
  }
  else {
